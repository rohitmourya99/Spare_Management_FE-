    ...
    <Layout title="BHEL Site Master Directory">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          ...
          sites.map((site) => (
            <Card key={site.id} className="hover:border-brand-500/40 transition-all">
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                {site.siteCode && (
                  <span className="text-[10px] font-mono font-bold bg-slate-800 text-brand-400 px-2 py-0.5 rounded border border-slate-700">
                    {site.siteCode}
                  </span>
                )}
              </div>
              <p className="text-xs text-slate-400 mb-3 border-b border-slate-800/80 pb-2.5 leading-relaxed">
                {site.fullAddress || `${site.city || ''}, ${site.state || ''}`}
              </p>
              <div className="space-y-1.5 text-xs text-slate-300">
                <div className="flex items-center gap-2">
                  <User className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                  <span className="font-semibold text-slate-200">{site.contactPerson || '—'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Phone className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                  <span className="font-mono text-slate-400">{site.phone || '—'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Mail className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                  <span className="text-slate-400 truncate">{site.email || '—'}</span>
                </div>
              </div>
            </Card>
          ))
      ...
      <Modal isOpen={importModalOpen} onClose={() => setImportModalOpen(false)} title="Import BHEL SPOC Master Excel" maxWidth="md">
        <div className="space-y-4">
          ...
          >
            ...
            <p className="text-sm text-slate-400">Click to select SPOC Excel file</p>
            <p className="text-xs text-slate-600 mt-1">Supports .xlsx and .xls</p>
          </div>
          ...
          {isImporting && (
            <div className="flex items-center gap-3 p-3 bg-brand-500/10 border border-brand-500/20 rounded-xl">
              ...
              <span className="text-sm text-brand-300">Importing sites...</span>
            </div>
          )}
          ...
          {importSummary && !importSummary.error && (
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 space-y-2">
              <p className="text-sm font-semibold text-emerald-400 flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4" /> Sites Imported Successfully
              </p>
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="bg-slate-900/60 rounded-lg p-2">
                  <p className="text-slate-500">Imported / Created</p>
